    ...
    var body: some View {
        ZStack {
            RoundedRectangle(cornerRadius: 8)
                .fill(Color.fitRed.opacity(0.15))
...
func workoutIconName(for type: String) -> String {
    ...
    if t.contains("cardio dance")           { return "figure.dance" }
    if t.contains("triathlon")              { return "figure.run" }
    if t.contains("transition")             { return "figure.walk" }
    if t.contains("underwater diving")      { return "figure.open.water.swim" }
